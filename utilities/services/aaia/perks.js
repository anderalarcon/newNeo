import WorkshopEstrategiaImg from '../../../public/assets/Services/aaia/Especifics/analytics-results-image-service1.jpg'
import perkIcon1WorkshopEstrategia from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon1-service1.svg'
import perkIcon2WorkshopEstrategia from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon2-service1.svg'
import perkIcon3WorkshopEstrategia from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon3-service1.svg'

import MachineLearningImg from '../../../public/assets/Services/aaia/Especifics/analytics-results-image-service2.jpg'
import perkIcon1MachineLearning from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon1-service2.svg'
import perkIcon2MachineLearning from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon2-service2.svg'
import perkIcon3MachineLearning from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon3-service2.svg'

import MigracionGa4Img from '../../../public/assets/Services/aaia/Especifics/analytics-results-image-service3.jpg'
import perkIcon1MigracionGa4 from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon1-service3.svg'
import perkIcon2MigracionGa4 from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon2-service3.svg'
import perkIcon3MigracionGa4 from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon3-service3.svg'

import AdvancedAnalyticsImg from '../../../public/assets/Services/aaia/Especifics/analytics-results-image-service4.jpg'
import perkIcon1AdvancedAnalytics from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon1-service4.svg'
import perkIcon2AdvancedAnalytics from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon2-service4.svg'
import perkIcon3AdvancedAnalytics from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon3-service4.svg'

import AnaliticaDigitalImg from '../../../public/assets/Services/aaia/Especifics/analytics-results-image-service5.jpg'
import perkIcon1AnaliticaDigital from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon1-service5.svg'
import perkIcon2AnaliticaDigital from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon2-service5.svg'
import perkIcon3AnaliticaDigital from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon3-service5.svg'

import PredictiveAnalyticsImg from '../../../public/assets/Services/aaia/Especifics/analytics-results-image-service6.jpg'
import perkIcon1PredictiveAnalytics from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon1-service6.svg'
import perkIcon2PredictiveAnalytics from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon2-service6.svg'
import perkIcon3PredictiveAnalytics from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon3-service6.svg'

import PredictiveSalesImg from '../../../public/assets/Services/aaia/Especifics/analytics-results-image-service7.jpg'
import perkIcon1PredictiveSales from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon1-service7.svg'
import perkIcon2PredictiveSales from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon2-service7.svg'
import perkIcon3PredictiveSales from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon3-service7.svg'

import ChatbotsInteligenciaImg from '../../../public/assets/Services/aaia/Especifics/analytics-results-image-service8.jpg'
import perkIcon1ChatbotsInteligencia from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon1-service8.svg'
import perkIcon2ChatbotsInteligencia from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon2-service8.svg'
import perkIcon3ChatbotsInteligencia from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon3-service8.svg'
import perkIcon4ChatbotsInteligencia from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon4-service8.svg'
import perkIcon5ChatbotsInteligencia from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon5-service8.svg'

import SentimentAnalysisImg from '../../../public/assets/Services/aaia/Especifics/analytics-results-image-service9.jpg'
import perkIcon1SentimentAnalysis from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon1-service9.svg'
import perkIcon2SentimentAnalysis from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon2-service9.svg'
import perkIcon3SentimentAnalysis from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon3-service9.svg'

import IntegracionDatosImg from '../../../public/assets/Services/aaia/Especifics/analytics-results-image-service10.jpg'
import perkIcon1IntegracionDatos from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon1-service10.svg'
import perkIcon2IntegracionDatos from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon2-service10.svg'
import perkIcon3IntegracionDatos from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon3-service10.svg'

import AutomatizacionReportesImg from '../../../public/assets/Services/aaia/Especifics/analytics-results-image-service11.jpg'
import perkIcon1AutomatizacionReportes from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon1-service11.svg'
import perkIcon2AutomatizacionReportes from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon2-service11.svg'
import perkIcon3AutomatizacionReportes from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon3-service11.svg'
import perkIcon4AutomatizacionReportes from '../../../public/assets/Services/aaia/Especifics/analytics-results-icon4-service11.svg'

export const perksWorkshopEstrategia = {
  coverImg: WorkshopEstrategiaImg,
  title: '¿Qué podrás lograr con este servicio?',
  perks: [
    {
      text: 'Identificar el estadío de madurez en el uso de data de la organización.',
      icon: perkIcon1WorkshopEstrategia
    },
    {
      text: 'Alinear a los líderes y stakeholders en los hallazgos en un workshop estratégico',
      icon: perkIcon2WorkshopEstrategia
    },
    {
      text: 'Planificar un roadmap con proyectos de data que aseguren un impacto para el negocio.',
      icon: perkIcon3WorkshopEstrategia
    }
  ],
  contact: {
    text: 'Contáctanos',
    link: 'https://google.com'
  }
}

export const perksMachineLearning = {
  coverImg: MachineLearningImg,
  title: '¿Qué podrás lograr con este servicio?',
  perks: [
    {
      text: 'Identificar patrones y tendencias en los datos de la organización para mejorar la toma de decisiones y el rendimiento del negocio.',
      icon: perkIcon1MachineLearning
    },
    {
      text: 'Utilizar técnicas de analítica avanzada y machine learning para predecir y prevenir problemas y a su vez personalizar y mejorar la experiencia del cliente.',
      icon: perkIcon2MachineLearning
    },
    {
      text: 'Desarrollar nuevos productos y servicios basados en los datos de la organización para generar y aumentar la ventaja competitiva.',
      icon: perkIcon3MachineLearning
    }
  ],
  contact: {
    text: 'Contáctanos',
    link: 'https://google.com'
  }
}

export const perksMigracionGa4 = {
  coverImg: MigracionGa4Img,
  title: '¿Qué podrás lograr con este servicio?',
  perks: [
    {
      text: 'Contar con una medición mejorada de los activos digitales mediante las nuevas funcionalidades de Google Analytics 4.',
      icon: perkIcon1MigracionGa4
    },
    {
      text: 'Tener una configuración en Google Analytics 4 acorde a los objetivos del negocio.',
      icon: perkIcon2MigracionGa4
    },
    {
      text: 'Visibilizar los principales indicadores de comportamiento del usuario en el activo digital.',
      icon: perkIcon3MigracionGa4
    }
  ],
  contact: {
    text: 'Contáctanos',
    link: 'https://google.com'
  }
}

export const perksAdvancedAnalytics = {
  coverImg: AdvancedAnalyticsImg,
  title: '¿Qué podrás lograr con este servicio?',
  perks: [
    {
      text: 'Mejorar la toma de decisiones usando datos actualizados para identificar oportunidades y desafíos en el rendimiento del sitio web.',
      icon: perkIcon1AdvancedAnalytics
    },
    {
      text: 'Contar con una medición mejorada de los activos digitales mediante las nuevas funcionalidades de Google Analytics 4.',
      icon: perkIcon2AdvancedAnalytics
    },
    {
      text: 'Generar análisis segmentados con mayor impacto y optimizar el performance de las herramientas de publicidad.',
      icon: perkIcon3AdvancedAnalytics
    }
  ],
  contact: {
    text: 'Contáctanos',
    link: 'https://google.com'
  }
}

export const perksAnaliticaDigital = {
  coverImg: AnaliticaDigitalImg,
  title: '¿Qué podrás lograr con este servicio?',
  perks: [
    {
      text: 'Tener un modelo de medición de datos desarrollado en base al objetivo de tu negocio.',
      icon: perkIcon1AnaliticaDigital
    },
    {
      text: 'Visualizar los datos y KPI´s en un dashboard de crashlytics para mejorar la eficiencia en la toma de decisiones en tu empresa.',
      icon: perkIcon2AnaliticaDigital
    },
    {
      text: 'Visualizar los datos y KPI´s en un dashboard de crashlytics para mejorar la eficiencia en la toma de decisiones en tu empresa.',
      icon: perkIcon3AnaliticaDigital
    }
  ],
  contact: {
    text: 'Contáctanos',
    link: 'https://google.com'
  }
}

export const perksPredictiveAnalytics = {
  coverImg: PredictiveAnalyticsImg,
  title: '¿Qué podrás lograr con este servicio?',
  perks: [
    {
      text: 'Aumentar la taza de retención identificando a los clientes con mayor probabilidad de abandono.',
      icon: perkIcon1PredictiveAnalytics
    },
    {
      text: 'Aumentar la taza de conversión identificando clientes con características comunes para personalizar su experiencia.',
      icon: perkIcon2PredictiveAnalytics
    },
    {
      text: 'Optimizar la asignación de recursos clasificando los leads en una escala en relación al valor que tiene cada uno para la organización.',
      icon: perkIcon3PredictiveAnalytics
    }
  ],
  contact: {
    text: 'Contáctanos',
    link: 'https://google.com'
  }
}

export const perksPredictiveSales = {
  coverImg: PredictiveSalesImg,
  title: '¿Qué podrás lograr con este servicio?',
  perks: [
    {
      text: 'Clasificar y recomendar productos según previos comportamientos de venta para incrementar el carrito de compra.',
      icon: perkIcon1PredictiveSales
    },
    {
      text: 'Lograr un incremento de participación en el mercado, identificando tendencias para generar nuevos productos y ofertas. ',
      icon: perkIcon2PredictiveSales
    },
    {
      text: 'Predicción de la demanda futura para optimizar costos de manejo de stock y control de cadena de suministro más eficiente.',
      icon: perkIcon3PredictiveSales
    }
  ],
  contact: {
    text: 'Contáctanos',
    link: 'https://google.com'
  }
}

export const perksChatbotsInteligencia = {
  coverImg: ChatbotsInteligenciaImg,
  title: '¿Qué podrás lograr con este servicio?',
  perks: [
    {
      text: 'Chatbot implementado en tu activo digital e integrado con las herramientas requeridas.',
      icon: perkIcon1ChatbotsInteligencia
    },
    {
      text: 'Capacitaciones para el manejo y entrenamiento continuo del chatbot.',
      icon: perkIcon2ChatbotsInteligencia
    },
    {
      text: 'Desarrollo de un funnel de conversación para definir el flujo de los clientes desde el inicio de la conversación hasta el objetivo final.',
      icon: perkIcon3ChatbotsInteligencia
    },
    {
      text: 'Análisis de la conversaciones para poder extraer resúmenes, palabras claves y temas de conversación.',
      icon: perkIcon4ChatbotsInteligencia
    },
    {
      text: 'Entrenamiento y actualización del chatbot para que pueda responder más tipos de consultas.',
      icon: perkIcon5ChatbotsInteligencia
    }
  ],
  contact: {
    text: 'Contáctanos',
    link: 'https://google.com'
  }
}

export const perksSentimentAnalysis = {
  coverImg: SentimentAnalysisImg,
  title: '¿Qué podrás lograr con este servicio?',
  perks: [
    {
      text: 'Conocer cuáles son los factores que hacen que tu marca tenga una mala o buena reputación.',
      icon: perkIcon1SentimentAnalysis
    },
    {
      text: 'Incrementar la tasa de retorno de inversión en tus campañas de marketing.',
      icon: perkIcon2SentimentAnalysis
    },
    {
      text: 'Visualizar y analizar los sentimientos y emociones en un dashboard intuitivo para la toma de decisiones.',
      icon: perkIcon3SentimentAnalysis
    }
  ],
  contact: {
    text: 'Contáctanos',
    link: 'https://google.com'
  }
}

export const perksIntegracionDatos = {
  coverImg: IntegracionDatosImg,
  title: '¿Qué podrás lograr con este servicio?',
  perks: [
    {
      text: 'Conocer cuáles son los factores que hacen que tu marca tenga una mala o buena reputación.',
      icon: perkIcon1IntegracionDatos
    },
    {
      text: 'Incrementar la tasa de retorno de inversión en tus campañas de marketing.',
      icon: perkIcon2IntegracionDatos
    },
    {
      text: 'Visualizar y analizar los sentimientos y emociones en un dashboard intuitivo para la toma de decisiones.',
      icon: perkIcon3IntegracionDatos
    }
  ],
  contact: {
    text: 'Contáctanos',
    link: 'https://google.com'
  }
}

export const perksAutomatizacionReportes = {
  coverImg: AutomatizacionReportesImg,
  title: '¿Qué podrás lograr con este servicio?',
  perks: [
    {
      text: 'Automatizar la integración de fuentes externas para facilitar el análisis, visualización o activación.',
      icon: perkIcon1AutomatizacionReportes
    },
    {
      text: 'Tener un repositorio de datos centralizados para ser consumidos por los diferentes equipos de la organización.',
      icon: perkIcon2AutomatizacionReportes
    },
    {
      text: 'Ahorrar tiempo de procesamiento y cruce de fuentes de datos para realizar análisis, modelos o reportería.',
      icon: perkIcon3AutomatizacionReportes
    },
    {
      text: 'Reducción de trabajo manual para la construcción de reportería, ahorro de tiempo y costos asociados.',
      icon: perkIcon4AutomatizacionReportes
    }
  ],
  contact: {
    text: 'Contáctanos',
    link: 'https://google.com'
  }
}
